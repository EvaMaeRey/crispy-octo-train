...
subtitle: "a brief, gentle and whimsical first-look behind the curtains of layers"
...
::: {focus-on="cr-PLOT00"}
Maybe you get giddy thinking about `geom_bump` `geom_ridgelines`, or classic `geom_histogram`?
:::
...
::: {focus-on="cr-PLOT0"}
Well-composed layers afford more fluid analytic discovery. 
:::
...
In `geom_bar()` the `geom` is fixed, but `stat` and `position` are adjustable.  You can see that their defaults are `"count"` and `"stack"` in the geom_bar definition.
...
::: {focus-on="cr-PLOT2a"}
But, instead of using convenient `geom_bar()`, we can use the more generic `layer()` function - which is actually used under the hood to define all geom_ and stat_ functions.  
:::


We can reproduce the same behavior with `layer()`, but we must provide all three 'control operators'. `layer(geom = "bar", stat = "count", position = "stack")`.
...
::: {focus-on="cr-PLOT2c"}
Or, equivalently, we can simply name the underlying `ggproto` *objects*, e.g. `GeomBar`, `StatCount`, and position function, `position_stack()` .
:::
...
::: {focus-on="cr-PLOT2f"}
Reiteration:   *There are actually 'control operators' that define `geom_*` and `stat_*` function.  `geom`, `stat`, and `position`. *
:::
...
*Or use them directly, `layer(geom = GeomBar, stat = StatCount, position = position_stack())`*
...
::: {focus-on="cr-PLOT2d"}
Focus: *Lets look at one 'control operator', Stat, more closely.*
:::
...
::: {focus-on="cr-PLOT4a"}
Key point: *We might think Stat's job as doing heavy lifting - computation that the user might otherwise be responsible for before plotting.*
:::
