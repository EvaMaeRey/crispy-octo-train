...
In `geom_bar()` the `geom` is fixed, but `stat` and `position` are adjustable - their defaults are `"count"` and `"stack"`.
...
We can get the same behavior, but we must provide all three control operators... `layer(geom = "bar", stat = "count", position = "stack")`.
...
::: {focus-on="cr-PLOT2f"}
Reiteration:   *There are actually 'control operators' for every layer (geom_ or stat_ ) call: `geom`, `stat`, and `position`. *
...
::: {focus-on="cr-PLOT4"}
We can think about StatCount's job as doing some computation that the user might otherwise be responsible for. We use StatCount\$compute_group() to manually do this computation for us, in conjunction with StatIdentity in `layer` (leaves data alone) to explicitly show this work.
:::
